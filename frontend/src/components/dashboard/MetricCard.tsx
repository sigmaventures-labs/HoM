...
export interface MetricCardProps {
  title: string;
  value: string | number;
  change: number; // percent delta vs prior period
  target?: number; // optional percent target for rate metrics
  breakdown?: { [key: string]: number };
  trend: "up" | "down" | "flat";
  status: "good" | "warning" | "critical";
  onAskAI: (context: MetricContext) => void;
  sparkline?: { label: string | number; value: number }[]; // last-12-weeks or similar
}
...
export function MetricCard({
  ...
  const getStatusColor = () => {
    switch (status) {
      case "good":
        return "gradient-success text-success-foreground glow-success";
      case "warning":
        return "gradient-warning text-warning-foreground glow-warning";
      case "critical":
        return "gradient-destructive text-destructive-foreground glow-destructive";
      default:
  ...
  return (
    <Card className={`relative glass-effect transition-all duration-300 border-0 ${getStatusGlow()}`}>
      ...
      <CardContent className="space-y-6">
        ...
        <Button
          ...
          className="w-full gradient-primary transition-all duration-200 glow-primary border-0 text-white font-semibold"
        >
